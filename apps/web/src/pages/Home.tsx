    ...
    <>
      ...
      <div className="meta" style={{ margin: '4px 0 12px' }}>
        {res.isLoading ? 'searching…'
          ...
          : `most recent 100 encounters — search to reach the entire database (total of ${res.data?.total ?? '…'} matches)`}
      </div>
...
function MatchRow({ m }: { m: HomeMatch }) {
  return (
    <Link to={`/match/${m.match_id}`} className="matchrow">
      ...
      {/* ham demo indirme düğmesi kaldırıldı (2026-07-12): statikte zaten
          yoktu; kasa-sonrası maçlarda ham durmadığı için ölü düğme olurdu.
          API ucu (/matches/{id}/demo) duruyor — gerekirse elle çağrılır. */}
    </Link>
  );
}
