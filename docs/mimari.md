...
**LightGBM v2 adayı (`ml/boost.py`, 2026-07-07 — Faz D, ~52k rauntla eşik aşıldı).** Her (harita, taraf) çifti için ayrı çok-sınıflı LightGBM; bilgi seti team_buy taban çizgisiyle AYNI granülaritede (buy tipi + takımın recency-ağırlıklı geçmiş küme payları + log n_eff) — avantajı takımlar-arası genelleme (benzer stil parmak izlerinin öğrenilmiş havuzlaması). evaluate.py'nin zamansal sınavına altıncı yöntem olarak girer (`prediction_meta.logloss_lgbm`); kazandığı çiftlerde `lgbm_predictions` sunum tablosu doldurulur ve `/predict` oradan servis eder, kaybettiği/verisiz çiftlerde tablo boş kalır ve zincir dürüstçe team_buy→team→league'e düşer. Özellik önemleri `prediction_meta.lgbm_importance`'ta (ML Lab şeffaflık paneli). Eğitim deterministik (seed=42), örnek ağırlığı = zaman azalımı; <200 eğitim raundu olan çift yarışa girmez. Bağımlılık: lightgbm (lokal, API-yok kuralına uygun; macOS'ta brew libomp ister).

**Zaman ağırlığı (`ml/recency.py`, 2026-07-06).** Tüm frekans/olasılık modülleri (tendencies, predict/evaluate'in team/team_vs/team_style katmanları, setups, utility, templates) artık üstel zaman-azalımıyla hesaplanır: ağırlık `0.5^(gün_farkı / 90)`, referans arşivdeki **en son maçın** `played_at`'i (duvar saati değil — proje molada da ağırlıklar sabit kalır). Tarihi bilinmeyen maçlar en yeni kabul edilir (weight=1.0). İlke: eşik/gating kontrolleri (`MIN_ROUNDS`, `MIN_COUNT`, h2h ≥ 6 vb.) her zaman HAM (ağırlıksız) gözlem sayısına bakar; yalnız pay/olasılık/sıralama alanları (`share`, `shrunk_prob`, `prob`, `team_exec_templates.recency_score`) ağırlıklıdır. `observed`/`sample_size`/`n`/`count` kolonları bu yüzden hâlâ gerçek (ağırlıksız) sayıyı taşır — şeffaflık için.
